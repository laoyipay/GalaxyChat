...
contract Chat {
    ...
    mapping(bytes32 => message[]) public allMessages; // key : Hash(user1,user2)


    mapping(string => message[]) public allMessagesString; // key : Hash(user1,user2)
    ...
    function getMyFriendList2() external view returns(friend[] memory) {
        return userList[msg.sender].friendList;
    }


    function getMyFriendList(address myAddress) external view returns(friend[] memory) {
        return userList[myAddress].friendList;
    }
  ...
  function readMessage(address ownAddress,address friend_key) external view returns(message[] memory) {
        bytes32 chatCode = _getChatCode(ownAddress, friend_key);
